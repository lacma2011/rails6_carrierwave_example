...
describe("example to-do app", () => {
  beforeEach(() => {
    ...
    cy.exec(
      'docker exec $(docker ps -q -f "name=rails") bundle exec rake db:reset db:seed'
    );

    cy.exec(
      'docker exec $(docker ps -q -f "name=rails") bundle exec rake cleaning:clear_uploads_folder'
    );

    cy.exec(
      'docker exec $(docker ps -q -f "name=rails") bundle exec pumactl restart'
    );
  });
